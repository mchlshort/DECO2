...
Cost_EP_NET = [180, 180, 180]

numperiods = len(prd) + 1
period_data_dict = {}

for (i,D,L,EPI,CEP) in zip(prd, Demand, Limit, EP_NET_CI, Cost_EP_NET):    
...
def EP_Period(plant_data,period_data): 
    ...
    #This variable represents the minimum deployment of NETs for each period 
    ...
    model.cons.add(sum(((model.net_energy[s]) + (model.net_energy_CCS[s] * model.B[s])) for s in model.S) + model.EP_NET == model.period_data['Demand'])
    ...
    model.cons.add(sum((model.net_energy[s] * model.plant_data[s]['CI']) + (model.net_energy_CCS[s] * model.CI_RET[s]) for s in model.S)
               + (model.EP_NET * model.period_data['EP_NET_CI']) == model.new_emission)
    ...
    model.cons.add(sum((model.net_energy[s] * model.plant_data[s]['Cost']) + (model.net_energy_CCS[s] * model.plant_data[s]['Cost_CCS'] * model.B[s]) for s in model.S)
               + (model.EP_NET * model.period_data['Cost_EP_NET']) == model.sum_cost) 
...
def EP_Results(plant_data, period_data, period):
    ...
    print(' ')
    print('Minimum NET Deployment in', period, '=', round(model.EP_NET(), 2), 'TWh/y')
    print(' ') 
    ...
    for s in plant_data.keys():
        energy_planning.loc['NET', 'Fuel'] = 'EP_NET'
        ...
        energy_planning.loc['NET', 'Net Energy'] = round(model.EP_NET(),2)  
        energy_planning.loc[s, 'Carbon Load'] = round((model.net_energy[s]() * plant_data[s]['CI']) + (model.net_energy_CCS[s]() * model.CI_RET[s]()), 2)
        energy_planning.loc['NET', 'Carbon Load'] = round(model.EP_NET() * model.period_data['EP_NET_CI'], 2)
