...
i.e. total demand, emission limit
'''
...
source_period_1 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 1', index_col=0, nrows=5, usecols='A:I').to_dict()
data_period_1 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 1', index_col=0, header=8, nrows=2, usecols='A:B').to_dict()


source_period_2 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 2', index_col=0, nrows=5, usecols='A:I').to_dict()
data_period_2 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 2', index_col=0, header=8, nrows=2, usecols='A:B').to_dict()

source_period_3 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 3', index_col=0, nrows=5, usecols='A:I').to_dict()
data_period_3 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 3', index_col=0, header=8, nrows=2, usecols='A:B').to_dict()
...
def EP_Period(source_data,period_data): 
    ...
    for s in model.S:
        ...
        model.cons.add((model.source_data[s]['CI'] * (1 - model.source_data[s]['RR']) / (1 - model.source_data[s]['X'])) == model.CI_RET[s])
        ...
        model.cons.add(model.CCS[s] * (1 - model.source_data[s]['X']) * model.B[s] == model.net_energy_CCS[s])
    ...
    model.cons.add(sum((model.net_energy[s] * model.source_data[s]['CI']) + (model.net_energy_CCS[s] * model.CI_RET[s]) for s in model.S)
               + (model.NET * model.source_data[s]['NET_CI']) == model.new_emission)     
...
def EP_Results(source_data, period_data, period):
    ...
    for s in source_data.keys():
        ...
        energy_planning.loc['NET', 'Carbon Load'] = round(model.NET() * source_data[s]['NET_CI'], 2)
