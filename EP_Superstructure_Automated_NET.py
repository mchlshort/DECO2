...
#Carbon intensity of EP-NET for each period
EP_NET_CI = [-0.6, -0.6, -0.6]

#Cost of EP-NET for each period
Cost_EP_NET = [150, 150, 150]

#Carbon intensity of EC-NET for each period
EC_NET_CI = [-0.4, -0.4, -0.4]

#Cost of EC-NET for each period
Cost_EC_NET = [200, 200, 200]
...
for (i,D,L,EPI,CEP,ECI,CEC,CI,CC) in zip(prd, Demand, Limit, EP_NET_CI, Cost_EP_NET, EC_NET_CI, Cost_EC_NET, Comp_CI, Cost_Comp):    
...
def EP_Period(plant_data,period_data): 
    ...
    #This variable represents the minimum deployment of EP_NETs for each period 
    model.EP_NET = pyo.Var(domain = pyo.NonNegativeReals)
    ...
    model.cons.add(sum(((model.net_energy[s]) + (model.net_energy_CCS[s] * model.B[s])) for s in model.S) + model.comp + model.EP_NET == model.period_data['Demand'] + model.EC_NET) 
    ...
    model.cons.add(sum((model.net_energy[s] * model.plant_data[s]['CI']) + (model.net_energy_CCS[s] * model.CI_RET[s]) for s in model.S)
               ...
               + (model.EP_NET * model.period_data['EP_NET_CI'])
    ...
    model.cons.add(sum((model.net_energy[s] * model.plant_data[s]['Cost']) + (model.net_energy_CCS[s] * model.plant_data[s]['Cost_CCS'] * model.B[s]) for s in model.S)
               ...
               + (model.EP_NET * model.period_data['Cost_EP_NET'])
...
def EP_Results(plant_data, period_data, period):
    ...
    print(' ')
    print('Minimum EP_NET Deployment in', period, '=', round(model.EP_NET(), 2), 'TWh/y')
    print(' ') 
    print('Minimum EC_NET Deployment in', period, '=', round(model.EC_NET(), 2), 'TWh/y')
    print(' ') 
    ...
    for s in plant_data.keys():
        energy_planning.loc['EP_NET', 'Fuel'] = 'EP_NET'
        energy_planning.loc['EC_NET', 'Fuel'] = 'EC_NET'        
        ...
        energy_planning.loc['EP_NET', 'Net Energy'] = round(model.EP_NET(),2)
        energy_planning.loc['EC_NET', 'Net Energy'] = round(model.EC_NET(),2)
        ...
        energy_planning.loc['EP_NET', 'Carbon Load'] = round(model.EP_NET() * model.period_data['EP_NET_CI'], 2)
        energy_planning.loc['EC_NET', 'Carbon Load'] = round(model.EC_NET() * model.period_data['EC_NET_CI'], 2)
