...
Direct Optimization Energy Planning Scenario in Pyomo
...
file_name = r'DO_EP_CT_Base_User_Interface_Pakistan.xlsx'
...
def multiperiod_energy_planning(model, i):
    ...
    #This variable defines the perunit emissions price of a power plant s for period i
    model.emissions_unit_cost = pyo.Var(i, model.S, domain = pyo.NonNegativeReals, bounds = (0, 1000000), initialize = 0)
    ...
    #This constraint defines the mminimum limit for per unit emissions price to be paid by a power plant s for period i
    def Emissions_unit_cost_LB(model, i, s):
        if CT == 'ON':
            return (model.emissions_unit_cost[i,s]) >= (model.EP['Emissions Price LB'][i]) * model.A[i,s]
        elif CT == 'OFF':
            return pyo.Constraint.Skip
    
    model.Cons_70 = pyo.Constraint(i, model.S, rule = Emissions_unit_cost_LB)
    ...
    def Emissions_unit_cost_value(model, i, s):
        if CT == 'ON':
            return model.emissions_unit_cost[i,s] == model.A[i,s] * (model.emissions_unit_cost[i,s])
        elif CT == 'OFF':
            return pyo.Constraint.Skip
    
    model.Cons_71 = pyo.Constraint(i, model.S, rule = Emissions_unit_cost_value)
    ...
    model.Cons_72 = pyo.Constraint(i, model.S, rule = CT_emissions_UL)
    ...
    model.Cons_73 = pyo.Constraint(i, model.S, rule = CT_emissions_LL)
    ...
    model.Cons_74 = pyo.Constraint(i, model.S, rule = CO2_load_plant)
    ...
    model.Cons_75 = pyo.Constraint(i, model.S, rule = CT_emissions_tradable)
    ...
    model.Cons_76 = pyo.Constraint(i, model.S, rule = CT_plant_emissions_balance)
    ...
    model.Cons_77 = pyo.Constraint(i, model.S, rule = CT_rem_balance_emissions)
    ...
    model.Cons_78 = pyo.Constraint(i, model.S, rule = CT_trade_within_plants)
    ...
    model.Cons_79 = pyo.Constraint(i, model.S, rule = CT_plant_emissions_value)
    ...
    model.Cons_80 = pyo.Constraint(i, model.S, rule = CT_plant_UL_final)
    ...
    model.Cons_81 = pyo.Constraint(i, model.S, rule = CT_plant_LL_final)
    ...
    model.Cons_82 = pyo.Constraint(i, model.S, rule = CT_balance_emissions_trading)
    ...
    model.Cons_83 = pyo.Constraint(i, model.S, rule = CT_plant_new_UL)
    
    
    #This constraint puts a budget cap on carbon trading
    def CT_budget_cap (model, i, s):
        if CT == 'ON':
            return ((sum((model.CT_emissions[i,s] * model.emissions_unit_cost[i,s]) for s in model.S)) + (model.EP['CT Additional Budget'][i])) >= ((-1) * (sum((model.Emissions_left [i,s] * model.emissions_unit_cost[i,s]) for s in model.S)))
        elif CT == 'OFF':
            return pyo.Constraint.Skip
    
    model.Cons_84 = pyo.Constraint(i, model.S, rule = CT_budget_cap)
    ...
    model.Cons_85 = pyo.Constraint(i, model.S, rule = CT_periods)
    ...
    def CT_cost (model, i, s):
        if CT == 'ON':
            return model.CT_cost[i,s] == model.emissions_unit_cost[i,s] * (model.CT_emissions[i,s] + model.ET_plant_new [i,s] + model.ET_plant_final [i,s] + model.Emissions_left [i,s])
        elif CT == 'OFF':
            return pyo.Constraint.Skip
        
    model.Cons_86 = pyo.Constraint(i, model.S, rule = CT_cost)
    ...
    model.Cons_87 = pyo.Constraint(i, model.S, rule = CO2_selling)
    ...
    model.Cons_88 = pyo.Constraint(i, model.S, rule = Stored_emissions_limit)
    ...
    model.Cons_89 = pyo.Constraint(i, model.S, rule = Plant_earning)
    ...
    model.Cons_90 = pyo.Constraint(i, model.S, rule = Electricity_Prices)
    
    
    #This constraint defines the initial electricity price for first period and puts upper bound to electricity prices
    ...
    model.Cons_91 = pyo.Constraint(i, model.S, rule = Electricity_Price_P1)
    ...
    model.Cons_92 = pyo.Constraint(i, model.S, rule = total_CO2_load)
    ...
    model.Cons_93 = pyo.Constraint(i, model.S, rule = energy_cost)  
    ...
    model.Cons_94 = pyo.Constraint(i, model.S, rule = Plant_net_cost)
    ...
    model.Cons_95 = pyo.Constraint(i, model.S, rule = fuel_cost) 
    ...
    model.Cons_96 = pyo.Constraint(i, model.S, rule = annualized_capital_cost) 
    ...
    model.Cons_99 = pyo.Constraint(i, model.S, rule = Plant_profit_net)
    ...
    #Constraints 101 and 102 are to check the feasibility of the model, and it also performs maximization of profits 
    def objective_constraint(model, i):
        if flag == 'max_profit':
            return model.new_emission[i] <= model.EP['Limit'][i]
        else:
    ...
    model.Cons_102 = pyo.Constraint(i, rule = objective_constraint)
    ...
    #optcr: This option specifies a relative termination tolerance for use in solving MIP problems, default is 5E-8, set it between 0.01 and 0.05 for quick computation, having a low value for this gives better results
...
def multiperiod_energy_planning_results(model, i):
    ...
    for s in model.plant.keys():
        ...
        if CT == 'ON':
            energy_planning.loc[s, 'CT Emissions Price (mil USD / Mt)'] = round(model.emissions_unit_cost[i,s](), 3)
            ...
            energy_planning.loc[s, 'CT Cost Paid to government to buy emissions from previous period (mil USD / y)'] = round((model.ET_plant_new[i,s]() * model.emissions_unit_cost[i,s]()), 3)
            energy_planning.loc[s, 'CT Cost Paid by Plant to government (mil USD / y)'] = round((model.CT_emissions [i,s]() * model.emissions_unit_cost[i,s]()), 3)
            energy_planning.loc[s, 'CT Cost Paid/earned by each plant as a result of trading with each other (mil USD / y)'] = round(model.emissions_unit_cost[i,s]() * model.ET_plant_final [i,s](), 3)
            energy_planning.loc[s, 'Balance Emissions to be traded in next period'] = round(model.Emissions_left[i,s](), 3)
            energy_planning.loc[s, 'Price paid/earned for balance emissions (mil USD / y)'] = round((model.Emissions_left[i,s]() * model.emissions_unit_cost[i,s]()), 3)        
        else:
            pass
    ...
    if CT == 'ON':
        ...
        energy_planning.loc['TOTAL', 'CT Cost Paid to government to buy emissions from previous period (mil USD / y)'] = round(sum((model.ET_plant_new[i,s]() * model.emissions_unit_cost[i,s]())for s in model.S) , 3)
        energy_planning.loc['TOTAL', 'CT Cost Paid by Plant to government (mil USD / y)'] = round(sum((model.CT_emissions [i,s]() * model.emissions_unit_cost[i,s]()) for s in model.S), 3)
        energy_planning.loc['TOTAL', 'CT Cost Paid/earned by each plant as a result of trading with each other (mil USD / y)'] = round(sum((model.emissions_unit_cost[i,s]() * model.ET_plant_final [i,s]()) for s in model.S), 3)
        energy_planning.loc['TOTAL', 'Balance Emissions to be traded in next period'] = round(model.CT_balance_emissions[i](), 3)
        energy_planning.loc['TOTAL', 'Price paid/earned for balance emissions (mil USD / y)'] = round(sum((model.Emissions_left[i,s]() * model.emissions_unit_cost[i,s]()) for s in model.S) , 3)
    else:
        pass
    #Above lines are commented by the if/else statement if CT is turned off
