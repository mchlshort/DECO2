...
path = os.chdir(r'C:/Users/LENOVO/OneDrive - University of Nottingham Malaysia/The University of Nottingham/BC COP26 Trilateral Research Initiative/Python/Python Formulation/NET Source') 

cwd = os.getcwd()

source_period_1 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 1', index_col=0, nrows=2, usecols='A:E').to_dict()
data_period_1 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 1', index_col=0, header=4, nrows=5, usecols='A:B').to_dict()


source_period_2 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 2', index_col=0, nrows=2, usecols='A:E').to_dict()
data_period_2 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 2', index_col=0, header=4, nrows=5, usecols='A:B').to_dict()

source_period_3 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 3', index_col=0, nrows=2, usecols='A:E').to_dict()
data_period_3 = pd.read_excel(r'Raw Data.xlsx', sheet_name = 'Period 3', index_col=0, header=4, nrows=5, usecols='A:B').to_dict()
...
def EP_Results(source_data, period_data, period):
    ...
    results = open('Multiperiod Energy Planning Results.txt', 'a')
    print(' ', file = results)
    print('Time period', '=' , period, file = results)
    print(' ', file = results)
    print('Carbon Intensity of CCS Retrofitted sources i in', period, file = results)
    print(' ', file = results)
    
    for s in source_data.keys():    
        print(s, '=', round(model.CI_RET[s](), 3), 'TWh/y', file = results)

    print(' ', file = results)
    print('Extent of CCS retrofit on source i in', period, file = results)
    print(' ', file = results)
    
    for s in source_data.keys():    
        print(s, '=', round(model.CCS[s](), 2), 'TWh/y', file = results)

    print(' ', file = results)
    print('Net energy from source i without CCS retrofit in', period, file = results)
    print(' ', file = results)
    
    for s in source_data.keys():
        print(s, '=', round(model.net_energy[s](), 2), 'TWh/y', file = results)

    print(' ', file = results)    
    print('Net energy from source i with CCS retrofit in', period, file = results)
    print(' ', file = results)
    
    for s in source_data.keys():
        print(s, '=', round(model.net_energy_CCS[s](), 2), 'TWh/y', file = results)

    print(' ', file = results)  
    print('Minimum NET Deployment in', period, '=', round(model.NET(), 2), 'TWh/y', file = results)
    print(' ', file = results) 
    ...
    print('Energy Planning Table for', period, file = results)
    print(' ', file = results)
    print(energy_planning, file = results)
